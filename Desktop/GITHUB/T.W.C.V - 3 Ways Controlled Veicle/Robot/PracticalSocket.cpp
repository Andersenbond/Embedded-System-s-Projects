...
void CommunicatingSocket::send(char sendbyte) throw(SocketException) 
{
char send=sendbyte; //0 nao é usado
if (send>=128)
	{
	char pre=192; if(send>=128) {pre+=2;} if(send>=192) {pre+=1;send-=64;} 
	if (::send(sockDesc, &pre, 1, 0) < 0) {	throw SocketException("Send failed (send())", true);}
	}
if (::send(sockDesc, &send, 1, 0) < 0)        {	throw SocketException("Send failed (send())", true);}
}
