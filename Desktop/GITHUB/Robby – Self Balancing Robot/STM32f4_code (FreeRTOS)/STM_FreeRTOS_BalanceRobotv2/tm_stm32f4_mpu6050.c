...
TM_MPU6050_Result_t TM_MPU6050_Init(TM_MPU6050_t* DataStruct, TM_MPU6050_Device_t DeviceNumber, TM_MPU6050_Accelerometer_t AccelerometerSensitivity, TM_MPU6050_Gyroscope_t GyroscopeSensitivity) {
	...
	//TM_I2C_Write(MPU6050_I2C,DataStruct->Address, MPU6050_INT_ENABLE, 1<<0);		//interupt occurs when data is ready. The interupt routine is in the receiver.c file.	
	//TM_I2C_Write(MPU6050_I2C,DataStruct->Address, MPU6050_INT_PIN_CFG, 1<<4);		//interrupt status bits are cleared on any read operation
	...
	switch (GyroscopeSensitivity) {
	 ...
	 //TM_I2C_Write(MPU6050_I2C,DataStruct->Address, MPU6050_SIGNAL_PATH_RESET, 0x07);//reset gyro and accel sensor
