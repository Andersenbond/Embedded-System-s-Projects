...
void main(){
    //gic_init_cpu();
    printk ("FreeRTOS Running on CORE %d\n",cpu_get_current());
  //  ltzvisor_hw_init();
...
//#ifdef ENABLE_PRINTF
        ...
        //printk("**** Thread-Metric Basic Single Thread Processing Test **** Relative Time: %lu\n", relative_time);

        /* See if there are any errors.  */
        if (tm_basic_processing_counter == last_counter)
        {

            printk("ERROR: Invalid counter value(s). Basic processing thread died!\n");
        }
